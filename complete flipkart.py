...
# Function to display order placed details
def display_order_placed(product, address, payment_method):
    order_window = tk.Toplevel(root)
    order_window.title("Order Placed")
    order_window.geometry("400x400")
    
    tk.Label(order_window, text="Order Placed Successfully!", font=("Arial", 16)).pack(pady=20)
    tk.Label(order_window, text=f"Product: {product}", font=("Arial", 14)).pack(pady=10)
    tk.Label(order_window, text="Shipping Address:", font=("Arial", 14)).pack(pady=10)
    tk.Label(order_window, text=address, font=("Arial", 12)).pack(pady=10)
    tk.Label(order_window, text=f"Payment Method: {payment_method}", font=("Arial", 14)).pack(pady=10)
...
def finalize_order(product, address, payment_method):
    if address and payment_method:
        display_order_placed(product, address, payment_method)
    else:
